...
export const InboxIssueStatus = observer(function InboxIssueStatus(props: Props) {
  ...
  const isSnoozedDatePassed =
    inboxIssue.status === EInboxIssueStatus.SNOOZED && new Date(inboxIssue.snoozed_till ?? "") < new Date();
    ...
    >
      <div className={`flex items-center gap-1`}>
        ...
        <div className="font-medium text-11 whitespace-nowrap">
          {inboxIssue?.status === EInboxIssueStatus.SNOOZED && inboxIssue?.snoozed_till
            ? description
