...
export function SpreadsheetIssueRowLoader(props: { columnCount: number }) {
  return (
    <tr className="border-b border-subtle bg-surface-1">
      <td className="h-11 min-w-[28rem] z-[10] sticky start-0 flex items-center border-e-[0.5px] border-subtle bg-surface-1">
      ...
      {range(props.columnCount).map((colIndex) => (
        <td key={colIndex} className="h-11 w-full min-w-[8rem] border-e border-subtle ">
  ...
  return (
    <div className="horizontal-scroll-enable h-full w-full overflow-y-auto ">
      <table>
        <thead>
          <tr>
            <th className="h-11 min-w-[28rem] bg-surface-2 border-e border-subtle animate-pulse" />
            {range(10).map((index) => (
              <th key={index} className="h-11 w-full min-w-[8rem] bg-surface-2 border-e border-subtle animate-pulse" />
            ))}
          </tr>
        </thead>
        <tbody>
